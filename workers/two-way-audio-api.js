'use strict';
/**
 * Two-Way Audio HTTP API server (runs on the Fly.io media node VM).
 *
 * Exposes a minimal REST API so the browser frontend can start a talk
 * session, stream microphone PCM data to the camera's speaker, and tear
 * the session down.
 *
...
const server = http.createServer(async (req, res) => {
  ...
  // Routes: /api/audio/<cameraId>/<action>
...
server.listen(PORT, () => {
  console.log(`[two-way-audio] API server listening on port ${PORT}`);
});
