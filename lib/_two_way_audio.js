...
const { OptalkAudioAdapter } = require('./_optalk_audio');
...
async function createTwoWayAudioAdapter(camera) {
  ...
  switch (caps.protocol) {
    ...
    case 'optalk': {
      ...
      const decrypted = camera.rtsp_password_encrypted
      ...
      // Use new OptalkAudioAdapter with verified OPTalk protocol
      const adapter = new OptalkAudioAdapter(camera.ip, port);
      await adapter.login(camera.rtsp_username || '', password);
      ...
      return {
        ...
        async sendAudioFrame(pcmData) {
          await adapter.sendAudio(pcmData);
        },
