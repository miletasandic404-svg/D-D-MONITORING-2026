           ...
           create: {
            before: async (user, context) => {
              return {
                data: {
                  ...user,
                  organization_id: context?.organizationId || user.organization_id || null,
                  user_type: user.user_type || 'operator',
                  status: context?.status || user.status || 'invited',
                },
              };
            },
          },
 ...
 *
 * organizationId and status are passed through Better Auth's request
 * context (via the `context` option on signUpEmail) so they reach the
 * databaseHooks.user.create.before hook via request-local AsyncLocalStorage
 * — no shared mutable module state, safe under concurrent calls.
 */
