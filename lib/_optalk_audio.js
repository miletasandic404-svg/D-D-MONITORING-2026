...
/**
  ...
  class OptalkAudioAdapter {
    constructor(ip, port = DVRIP_PORT) {
      ...
      this._cleanupFns = [];
      this._persistentListenerAttached = false;
      ...
      this.socket.on('data', (data) => {
        ...
        try {
          ...
          if (resp.success) {
            ...
            this._attachPersistentListener();
            resolve({ success: true });
          } else {
  ...
  /**
   * Attach persistent data listener to handle audio responses (msg 1433)
   * and other unsolicited messages from the camera.
   */
  _attachPersistentListener() {
    if (this._persistentListenerAttached || !this.socket) return;
    ...
    const listener = (data) => {
      ...
      while (this._responseBuffer.length >= 20) {
        const msgId = this._responseBuffer.readUInt16LE(14);
        const len = this._responseBuffer.readUInt32LE(16);

        if (this._responseBuffer.length < 20 + len) {
          break; // Need more data
        }

        const frameData = this._responseBuffer.subarray(20, 20 + len);
        this._responseBuffer = this._responseBuffer.subarray(20 + len);

        // Handle audio response (1433) - discard/acknowledge
        if (msgId === MSG_OPTALK_AUDIO_RESPONSE) {
          // Camera acknowledged audio frame - nothing to do
          continue;
        }

        // Other messages (keepalive responses, etc.) - log but don't block
        // console.log(`[optalk] Unsolicited msg ${msgId} len=${len}`);
      }
    };

    this.socket.on('data', listener);
    this._persistentListenerAttached = true;
