...
let loadError = null;
let onnxRuntimeLoader = () => require('onnxruntime-node');

// ── ONNX Runtime lazy load ──────────────────────────────────────────

function getOnnxRuntime() {
  try {
    const ort = onnxRuntimeLoader();
    ortAvailable = true;
    return ort;
  } catch (err) {
    ortAvailable = false;
...
module.exports = {
  ...
  _internal: {
    ...
    nms,
    computeIoU,
    // Unit tests inject a mock without requiring the optional native module.
    // Production continues to use the default loader above.
    setOnnxRuntimeLoaderForTest(loader) {
      onnxRuntimeLoader = loader;
      ortSession = null;
      ortAvailable = false;
      modelLoaded = false;
      loadError = null;
