 ...
 * @param {number} [sessionId=0] - Session ID (defaults to 0 for login)
 * @returns {Buffer} Complete V5.00 message
 */
function buildFrame(msgId, jsonObj, sessionId = 0) {
  const body = Buffer.from(JSON.stringify(jsonObj), 'utf8');
  const h = Buffer.alloc(20);
  h[0] = 0xFF;
  h[1] = REQUEST_TYPE; // 0x00 = V5.00 request
  h.writeUInt32LE(sessionId >>> 0, 4); // session ID (use >>> 0 for unsigned)
...
function buildKeepaliveMessage(sessionId) {
  return buildFrame(MSG_KEEPALIVE, {}, sessionId);
}
...
function buildOptalkClaimMessage(sessionId) {
  return buildFrame(MSG_OPTALK_CLAIM, {}, sessionId);
}
...
function buildOptalkStartMessage(sessionId) {
  ...
  return buildFrame(MSG_OPTALK_START, jsonObj, sessionId);
}
