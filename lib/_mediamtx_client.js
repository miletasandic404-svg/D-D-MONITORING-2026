...
async function addOrUpdateCameraPath(cameraId, rtspUrl) {
  ...
  const body = { source: rtspUrl };
  if (rtspUrl !== 'publisher') {
    body.sourceOnDemand = true;
  }
  try {
    return await request('POST', `/v3/config/paths/add/${encodeURIComponent(cameraId)}`, body);
  } catch (err) {
    if (err.status === 400 && err.body && err.body.error === 'path already exists') {
      await request('DELETE', `/v3/config/paths/delete/${encodeURIComponent(cameraId)}`);
      return await request('POST', `/v3/config/paths/add/${encodeURIComponent(cameraId)}`, body);
    }
    throw err;
  }
