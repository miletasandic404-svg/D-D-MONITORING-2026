...
async function findPaymentByProviderIdentifiers(clientOrDb, {
  ...
  const sql = `SELECT *
     ...
     LIMIT 1`;
  const params = [provider, providerPaymentId, providerOrderId, providerCaptureId];

  if (clientOrDb) {
    const { rows } = await clientOrDb.query(sql, params);
    return rows[0] || null;
  }

  const { rows } = await db.queryAsPlatformAdmin(sql, params);
  return rows[0] || null;
}
...
function assertPaymentOwnership(payment, auth) {
  if (!payment) {
    const err = new Error('Payment checkout record not found');
    err.statusCode = 409;
    throw err;
  }
  ...
  if (payment.organization_id) {
    if (!auth?.organizationId) {
      const err = new Error('Authentication required for organization payment');
      err.statusCode = 401;
      throw err;
    }
    if (payment.organization_id !== auth.organizationId) {
      const err = new Error('Payment record belongs to a different organization');
      err.statusCode = 403;
      throw err;
    }
  } else if (auth?.organizationId) {
    const err = new Error('Payment record is not assigned to this organization');
    err.statusCode = 403;
    throw err;
  }

  if (payment.user_id && auth?.userId && payment.user_id !== auth.userId) {
    const err = new Error('Payment record belongs to a different user');
    err.statusCode = 403;
    throw err;
  }
}
...
async function activatePaymentForOrganization({
  ...
  // Security: prevent cross-organization payment activation
  if (payment.organization_id && payment.organization_id !== organizationId) {
    const err = new Error('Payment record belongs to a different organization');
    err.statusCode = 403;
    throw err;
  }
...
async function reconcileStripeIntent({
  ...
  const existingPayment = await findPaymentByProviderIdentifiers(null, {
    provider: 'stripe',
    providerPaymentId: parsed.paymentIntentId,
  });
  assertPaymentOwnership(existingPayment, auth);
