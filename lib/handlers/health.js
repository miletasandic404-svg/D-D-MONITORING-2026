...
module.exports = async (req, res) => {
    ...
    try {
      ...
      const nodeCounts = await run(
        `SELECT 
          ...
          count(*) FILTER (WHERE status = 'offline') AS offline,
          count(*) FILTER (WHERE status IS NULL) AS unknown
        FROM media_nodes${isPlatformAdmin ? '' : ' WHERE organization_id = $1'}`,
        orgParams,
      );
  ...
  if (db.hasDatabase) {
    try {
      ...
      logger.error('[health] DB connectivity check failed', { error: err.message });
    }
    ...
    if (checks.database.connected) {
      try {
        ...
        logger.error('[health] organizations table check failed (migrations not run?)', { error: err.message });
      }
    }
  }
