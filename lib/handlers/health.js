...
async function probeApiHealth(dbRef) {
  // Read configuration at probe time. `db.hasDatabase` is intentionally a
  // module-load-time pool construction detail, so using it here would make
  // this health result depend on import order and hide a configured-but-down
  // database as merely "missing env".
        ...
        `SELECT 
          ...
          count(*) FILTER (
            WHERE status = 'offline'
              OR ((status = 'active' OR status IS NULL)
