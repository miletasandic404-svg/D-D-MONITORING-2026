...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const cam = await db.queryAsOrg(
      auth.organizationId,
      'SELECT id, site_id FROM cameras WHERE id = $1 AND organization_id = $2',
    ...
    if (auth.userType !== 'org_admin' && auth.userType !== 'platform_admin') {
      const hasAccess = await canAccessCamera(auth, camera_id);
      if (!hasAccess) {
        return sendError(res, 403, 'You are not authorized to create snapshots for this camera');
      }
    }
    ...
    const debugRequested = req.query?.debug === 'true';

    if (debugRequested && isPlatformAdmin) {
