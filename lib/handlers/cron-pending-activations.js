...
module.exports = async (req, res) => {
  ...
  const secret = getCronSecret();
  if (!secret) {
    console.error('[cron-pending-activations] CRON_SECRET is not set — endpoint disabled');
  ...
  try {
    ...
    console.log(
      `[cron-pending-activations] done: ${results.length} processed, ` +
    ...
    console.error('[cron-pending-activations] error:', err.message);
