...
const logger = makeLogger('handler-verify-stream-token');
...
module.exports = async (req, res) => {
  ...
  const cameraId = rawPath.replace(/^\//, '').replace(/\/.*$/, '');

  // TEMPORARY DEBUG LOGGING — remove after diagnosis
  const maskedQuery = (query || '').replace(/(token=)[^&]+/g, '$1<MASKED>');
  console.log('AUTH DEBUG:', JSON.stringify({ path: rawPath, query: maskedQuery, action }));
  ...
  try {
    ...
    logger.error('POST /api/verify-stream-token error', { error: err.message });
    Sentry.captureException(err);
    // Fail closed: deny access on any error (DB down, query error, etc.)
    res.status(401).end();
  }
};
