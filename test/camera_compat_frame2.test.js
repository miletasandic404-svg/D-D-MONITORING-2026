...
require('child_process').spawn = (cmd, args) => {
  console.log('MOCK SPAWN CALLED:', cmd, args[args.length - 1]);
  ...
  return {
    stdout: {
      on: (event, cb) => {
        console.log('MOCK stdout.on:', event);
...
describe('Camera Compatibility Gate', () => {
  ...
  test('debug extractFrameFromRtsp', async () => {
    ...
    console.log('extractFrameFromRtsp type:', typeof extractFrameFromRtsp);
    console.log('extractFrameFromRtsp source:', extractFrameFromRtsp.toString().slice(0, 200));
