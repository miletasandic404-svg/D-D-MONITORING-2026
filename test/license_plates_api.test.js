...
/**
 * Tests for api/license-plates/index.js — License Plate Recognition API.
 *
 * Verifies:
...
describe('api/license-plates — License Plate Recognition', () => {
  beforeEach(() => {
    resetFakes();
    auditCalls = [];
  });

  describe('GET /api/license-plates', () => {
    ...
    test('authenticated GET → returns own-org plates', async () => {
      dbScript = (text) => {
        if (text.includes('FROM known_plates')) {
          return { rows: [{ id: 'p1', plate_number: 'BG-123-AB' }] };
        }
        return { rows: [], rowCount: 0 };
      };
      const req = makeReq({ method: 'GET' });
      ...
      assert.equal(res.body.known_plates.length, 1);
    });
    ...
    test('GET scoped to authenticated org', async () => {
      ...
      const queryCall = queryCalls.find((c) => c.text.includes('FROM known_plates'));
      assert.equal(queryCall.orgId, 'org-1');
    });
  });

  describe('POST /api/license-plates/enroll', () => {
    test('unauthenticated POST → 401', async () => {
      authResponse = null;
      const req = makeReq({ method: 'POST', body: { plate_number: 'BG-123-AB' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('org_admin can enroll → success', async () => {
      authResponse = { userId: 'user-1', organizationId: 'org-1', userType: 'org_admin' };
      dbScript = (text) => {
        if (text.includes('INSERT INTO known_plates')) {
          return { rows: [{ id: 'p1', plate_number: 'BG-123-AB' }] };
        }
        return { rows: [], rowCount: 0 };
      };
      ...
      assert.equal(res.body.plate.plate_number, 'BG-123-AB');
    });
    ...
    test('empty plate_number → validation error', async () => {
      const req = makeReq({ method: 'POST', body: { plate_number: '' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('invalid status → validation error', async () => {
      const req = makeReq({ method: 'POST', body: { plate_number: 'BG-123-AB', status: 'invalid' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('organization_id from auth context', async () => {
      ...
      const insertCall = queryCalls.find((c) => c.text.includes('INSERT INTO known_plates'));
    ...
    test('organization_id in body is rejected by strict schema', async () => {
      const req = makeReq({ method: 'POST', body: { plate_number: 'BG-123-AB', organization_id: 'malicious-org' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('audit log is created on enroll', async () => {
      ...
      assert.equal(auditCalls[0].action, 'plate.enroll');
    });
  });

  describe('PUT /api/license-plates/:id', () => {
    test('admin can update own-org plate → success', async () => {
      dbScript = (text) => {
        if (text.includes('UPDATE known_plates')) {
          return { rows: [{ id: 'p1', status: 'blocked' }] };
        }
        return { rows: [], rowCount: 0 };
      };
      const req = makeReq({ method: 'PUT', query: { id: 'p1' }, body: { status: 'blocked' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('cross-org ID → 404', async () => {
      ...
      const req = makeReq({ method: 'PUT', query: { id: 'other-org-plate' }, body: { status: 'blocked' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('audit log is created on update', async () => {
      ...
      assert.ok(auditCalls.length > 0);
      assert.equal(auditCalls[0].action, 'plate.update');
    });
  });

  describe('DELETE /api/license-plates/:id', () => {
    test('admin can delete own-org plate → success', async () => {
      dbScript = (text) => {
        if (text.includes('DELETE FROM known_plates')) {
          return { rows: [{ id: 'p1' }] };
        }
        return { rows: [], rowCount: 0 };
      };
      const req = makeReq({ method: 'DELETE', query: { id: 'p1' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('cross-org ID → 404', async () => {
      ...
      const req = makeReq({ method: 'DELETE', query: { id: 'other-org-plate' } });
      const res = makeRes();
      await handler(req, res);
    ...
    test('audit log is created on delete', async () => {
      ...
      assert.ok(auditCalls.length > 0);
      assert.equal(auditCalls[0].action, 'plate.delete');
    });
  });
});
